...
$PAGE->set_url('/local/augmented_teacher/mergedmessages.php', array(
        'page' => $page,
        'perpage' => $perpage,
        'mode' => $mode,
...
$baseurl = new moodle_url('/local/augmented_teacher/mergedmessages.php', array(
...
if ($mycourses = enrol_get_my_courses()) {
    $courselist = array();
    $popupurl = new moodle_url('/local/augmented_teacher/mergedmessages.php?roleid='.$roleid.'&sifirst=&silast=');
    ...
    if ($totalcount < 1) {
        ...
        if ($matchcount > 0) {
            ...
            foreach ($userlist as $user) {
                ...
                $row->cells[1]->text = $OUTPUT->container(fullname($user,
                    has_capability('moodle/site:viewfullnames', $context)), 'username');
                ...
                if ($user->maildisplay == 1 or ($user->maildisplay == 2 and ($course->id != SITEID) and !isguestuser()) or
                    ...
                    $row->cells[1]->text .= get_string('email').
                        get_string('labelsep', 'langconfig').
                        html_writer::link("mailto:$user->email", $user->email) . '<br />';
                }
                foreach ($extrafields as $field) {
                    if ($field === 'email') {
                ...
                if (!isset($hiddenfields['lastaccess'])) {
                    if ($user->lastaccess) {
                        $row->cells[1]->text .= get_string('lastaccess').get_string('labelsep', 'langconfig').
                ...
                if (has_capability('moodle/site:viewreports', $context)
                    or has_capability('moodle/user:viewuseractivitiesreport', $usercontext)) {
                    $links[] = html_writer::link(
                        new moodle_url('/course/user.php?id='. $course->id .'&user='. $user->id),
                        get_string('activity')
                    );
                }
                ...
                if ($bulkoperations) {
                    ...
                    $row->cells[2]->text .= '<br /><input type="checkbox" class="usercheckbox" name="user'.
                        $user->id.'" ' .$checked .'/> ';
                }
        ...
        foreach ($userlist as $user) {
            ...
            if ($piclink = ($USER->id == $user->id || has_capability('moodle/user:viewdetails', $context)
                ...
                $profilelink = '<strong><a href="'.$CFG->wwwroot.'/user/view.php?id='.
                    $user->id.'&amp;course='.$course->id.'">'.fullname($user).'</a></strong>';
            } else {
...
if ($bulkoperations) {
    ...
    echo '<div><input type="submit" value="'.get_string('messageselectadd').'" /></div>';
    ...
    $PAGE->requires->js_init_call('M.local_augmented_teacher.init_participation', null, false, $module);
}
