...
class MoodleQuickForm_time_selector extends MoodleQuickForm_group {
     ...
     */
    public function MoodleQuickForm_time_selector($elementName = null, $elementLabel = null, $options = array(), $attributes = null) {
    ...
    function _createElements() {
        ...
        $this->_elements[] = $this->createFormElement('static', 'from', '', get_string('from'));
        $this->_elements[] = $this->createFormElement('select', 'hour_from', get_string('hour', 'form'), $hours,
            $this->getAttributes(), true);
        $this->_elements[] = $this->createFormElement('select', 'minute_from', get_string('minute', 'form'), $minutes,
            $this->getAttributes(), true);

        $this->_elements[] = $this->createFormElement('static', 'to', '', get_string('to'));
        $this->_elements[] = $this->createFormElement('select', 'hour_to', get_string('hour', 'form'), $hours,
            $this->getAttributes(), true);
        $this->_elements[] = $this->createFormElement('select', 'minute_to', get_string('minute', 'form'), $minutes,
            $this->getAttributes(), true);
        ...
        switch ($event) {
            case 'updateValue':
                ...
                if (!is_array($value)) {
                    ...
                    $minutefrom = (int)gmdate("i", $arr[0]);
                    $minutefrom -= $minutefrom % $this->_options['step'];
                    $minuteto = (int)gmdate("i", $arr[1]);
                    $minuteto -= $minutefrom % $this->_options['step'];
                    $value = array(
                        'hour_from' => (int)gmdate('G', $arr[0]),
                        'minute_from' => $minutefrom,
                        'hour_to' => (int)gmdate('G', $arr[1]),
                        'minute_to' => $minuteto
                    );

                    // If optional, default to off, unless a date was provided.
                    if ($this->_options['optional']) {
                        $value['enabled'] = $arr[1] != 0;
                    }
                } else {
    ...
    function exportValue(&$submitValues, $assoc = false) {
        ...
        if (count($valuearray)){
            ...
            $value = array(
                'from' => $valuearray['hour_from']*60*60 + $valuearray['minute_from']*60,
                'to' => $valuearray['hour_to']*60*60 + $valuearray['minute_to']*60
            );
...
MoodleQuickForm::registerElementType('time_selector', $CFG->dirroot."/local/augmented_teacher/classes/timeselector.php", 'MoodleQuickForm_time_selector');