...
$thispageurl = new moodle_url('/local/augmented_teacher/notloggedinreminders_delete.php', array('id' => $id));
...
if ($process) {
    ...
    echo $OUTPUT->header();
    echo html_writer::tag('h1', get_string('notloggedinreminderdelete', 'local_augmented_teacher'), array('class' => 'page-title'));
    echo $OUTPUT->confirm(
        html_writer::div(
        ...
        new moodle_url('/local/augmented_teacher/notloggedinreminders_delete.php', array('id' => $id, 'process' => 1)),
        $returnurl
    );
    echo $OUTPUT->footer();
}