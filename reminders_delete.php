...
$thispageurl = new moodle_url('/local/augmented_teacher/reminders_delete.php', array('id' => $id));
...
if ($process) {
    ...
    $event = \local_augmented_teacher\event\reminder_deleted::create(array(
        ...
        'other' => array('cmid' => $reminder->cmid),
        'context' => $context
    ));
    $event->trigger();
    redirect($returnurl);
    ...
    echo $OUTPUT->header();
    echo html_writer::tag('h1', get_string('reminderdelete', 'local_augmented_teacher'), array('class' => 'page-title'));
    echo $OUTPUT->confirm(
        html_writer::div(
        ...
        new moodle_url('/local/augmented_teacher/reminders_delete.php', array('id' => $id, 'process' => 1)),
        $returnurl
    );
    echo $OUTPUT->footer();
}